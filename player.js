...
function getWatchHistory() {
    return getStoredItems(WATCH_HISTORY_KEY);
}

function setWatchHistory(id, type, season, episode) {
    let history = getWatchHistory();
    history = history.filter(item => item.id !== id);
    const historyItem = { id, type };
    if (type === 'tv') {
...
function getLastWatched(showId) {
    ...
    return history.find(item => item.id === showId) || null;
}
...
async function initializePlayerPage() {
    ...
    if (mediaType === 'tv') {
        if (seasonFromUrl && episodeFromUrl) {
            // If user clicked a specific episode, use that.
            const startSeason = parseInt(seasonFromUrl);
            const startEpisode = parseInt(episodeFromUrl);
            setupTvPlayer(id, details, startSeason, startEpisode);
        } else {
            // Otherwise, check for watch history.
            ...
            if (lastWatched) {
                ...
                // No history and no specific episode clicked, start from beginning.
                setupTvPlayer(id, details, 1, 1);
            }
        }
    } else { 
...
async function updateVideoPlayer(id, season, episode) {
    ...
    if (isTv) {
        setWatchHistory(id, 'tv', season, episode);
    } else {
        setWatchHistory(id, 'movie');
    }

    const embedUrl = `https://vidsrc.to/embed/${currentTvState.type}/${id}${isTv ? `/${season}-${episode}` : ''}`;
