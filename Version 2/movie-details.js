...
const playMovieBtn = document.getElementById('playMovieBtn'); // Corrected ID
...
async function initializeDetailsPage() {
    ...
    if (data) {
        ...
        mainCrew.forEach(member => {
            ...
            crewMember.innerHTML = `<p class="font-bold">${member.name}</p><p class="text-sm text-gray-400">${member.job}</p>`;
...
function createUserScoreCircle(score) {
    ...
    if (percentage < 70) strokeColor = '#facc15'; // yellow-400
    if (percentage < 40) strokeColor = '#f87171'; // red-400
